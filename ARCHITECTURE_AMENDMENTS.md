...
| 4 | Introduce a seven-state Hermes lifecycle (`pending_decision`, `pending_review`, `applying`, `applied`, `dismissed`, `failed`, `reverted`). | **Accepted target**, not current runtime. | Current persistence and domain contract uses `pending_review`, `applied`, and `dismissed`; see `005_hermes_events.sql`, `HermesEvent.ts`, approve/dismiss use cases and tests. Reconciliation is tracked by [issue #178](https://github.com/ylazakovich/hermes-marketdesk/issues/178). |
...
The implemented lifecycle remains the three-state persistence contract until issue #178 changes the database, API, domain and UI atomically. The seven-state model above is an accepted target only.
