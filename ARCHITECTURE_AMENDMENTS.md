...
| 4 | Introduce a seven-state Hermes lifecycle (`pending_decision`, `pending_review`, `applying`, `applied`, `dismissed`, `failed`, `reverted`). | **Implemented**. | `HermesEvent.ts`, shared API types, migration `020_hermes_event_lifecycle.sql`, approve/dismiss/decision use cases, UI badge mapping and transition/API tests; delivered by [issue #178](https://github.com/ylazakovich/hermes-marketdesk/issues/178). |
