...
| Architecture truth and review decisions | `ARCHITECTURE.md` is canonical and distinguishes implemented runtime contracts from accepted targets. Amendments are historical decisions only. Runtime scheduling uses Bull/Redis repeatable jobs; database evolution uses ordered idempotent SQL; the expanded Hermes lifecycle remains a target. | `ARCHITECTURE.md` §§7, 10, 13; `ARCHITECTURE_AMENDMENTS.md`; migration runner, scheduler and related tests | Consolidated by [#179](https://github.com/ylazakovich/hermes-marketdesk/issues/179); lifecycle delivery remains [#178](https://github.com/ylazakovich/hermes-marketdesk/issues/178) |
