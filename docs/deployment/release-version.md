...
4. permits only detached Compose `up`, pins the repository Compose file/project directory, preserves Compose's existing directory-derived project identity, and always adds `--build`;
...
The command exits before invoking Compose when `HEAD` is not an exact valid release tag or the checkout is dirty. The application reads only the image file; runtime environment values cannot relabel an old image as a newer release. Do not manually persist a release tag in `.env`.
