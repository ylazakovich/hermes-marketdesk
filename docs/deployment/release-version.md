...
An interrupted or failed post-spawn deployment deliberately leaves `/tmp/marketdesk-release-<uid>/marketdesk.lock` in place, including its mode-`0600` environment snapshot. The single host-wide MarketDesk lock serializes every checkout that could mutate the globally named `marketdesk-*` containers, and retries fail closed regardless of checkout basename or `TMPDIR`. Verify that no Docker Compose/build process remains before manually removing the lock directory and retrying; never automate stale-lock removal.
