...
An interrupted or failed post-spawn deployment deliberately leaves `/tmp/marketdesk-release-<uid>/<project>.lock` in place, including its mode-`0600` environment snapshot. Retries fail closed regardless of `TMPDIR`. Verify that no Docker Compose/build process for that project remains before manually removing the lock directory and retrying; never automate stale-lock removal.
